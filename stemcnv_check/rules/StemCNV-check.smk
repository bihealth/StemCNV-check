...
def get_target_files():
    ...
    if TARGET == 'complete':
        return expand(
            ...
            expand(get_cnv_vcf_output(config['settings']['make_cnv_vcf']['mode']), sample_id = all_samples)
    ...
    # Target CBS
    if TARGET == 'CBS':
        return expand(os.path.join(DATAPATH, "{sample_id}", "{sample_id}.CBS.tsv"), sample_id = all_samples)
    ...
    if TARGET == 'SNP-probe-data':
        return expand(
            os.path.join(DATAPATH, "{sample_id}", "{sample_id}.processed-data.tsv"), sample_id = all_samples
        ) + \
