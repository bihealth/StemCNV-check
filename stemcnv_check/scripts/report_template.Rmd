...
raw_LRR_BAF <- file.path(datapath, use_ids, paste0(use_ids, '.annotated-SNP-data.', plot.probe.filter.setting, '-filter.vcf.gz')) %>%
  ...
  #   file.path(datapath, use_ids, paste0(use_ids, '.processed-data.tsv')) %>%
  # lapply(read_raw) %>%
  # bind_rows() %>%
...
SNP.genotype.distances <- file.path(datapath, snp.ids, 
                                    paste0(snp.ids, '.annotated-SNP-data.', cluster.probe.filter.setting, '-filter.vcf.gz')) %>%
    lapply(
        parse_snp_vcf,
        format_fields = c('GT'),
        apply_filter = !ignore.filter
        ) %>%
    bind_ranges() %>%
    as_tibble() %>%
    mutate(NumericGT = ifelse(GT == './.', NA, str_count(GT, '1'))) %>%
    select(-end, -width, -strand, -REF, -ALT, -QUAL, -FILTER, -GT) %>%
    pivot_wider(names_from = sample_id, values_from = NumericGT, values_fill = NA) %>%
    filter(if_all(everything(), ~!is.na(.))) %>%
    dplyr::select(-seqnames, -start, -ID) %>%
    t() %>%
...
cnv_calls <- lapply(file.path(datapath, use_ids, paste0(use_ids, '.combined-cnv-calls.tsv')),
  ...
  mutate(
    Call_label = case_when(
            ...
            Check_Score >= report.setting('check_score.critical')   ~ 'Critical',
            Check_Score >= report.setting('check_score.reportable') ~ 'Reportable',
            TRUE                         				              ~ ''
    )
  )
...
not_enabled_label <- c('dummy',
                       ifelse(report.setting('check_score.critical') == 'NA', 'critical', 'dummy'),
                       ifelse(report.setting('check_score.reportable') == 'NA', 'reportable', 'dummy')
...
cnv_calls <- cnv_calls %>%
  ungroup() %>%
  dplyr::rename(Chr = seqnames, Size = width, Start = start, End = end) %>%
  mutate(CNV_type = factor(CNV_type),
         reference_overlap = factor(reference_overlap),
  ) %>%
  arrange(desc(Check_Score), desc(Size), Chr, Start)
...
if (any(!is.na(regions)) & length(regions)>0) {
    ...
    fake_call_tb <- tibble(
        ...
        mutate(#Chr = ifelse(use_chr_str,  paste0('chr', Chr), Chr),
                   Chr = mapSeqlevels(Chr, seqlevelsStyle(cnv_calls$Chr)),
                   i = row_number(),
                   region_name = ifelse(is.na(region_name), paste('Region', i), region_name),
                   Check_Score = 0,
                   ID = region_name,
        ) %>%
        bind_rows(get_expected_final_tb(seqlevelsStyle(cnv_calls$Chr)))
