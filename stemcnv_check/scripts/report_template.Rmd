...
make_CNV_plot_section <- function(call.table, plotsection = 'denovo') {
    ...
    for (i in call.table$i) {
        ...
        row <- call.table[i,]
        res <- make_call_plot(
            row, raw_LRR_BAF, cnv_calls, gr_genes, gr_info, sample_headers,
...
paste(
    ...
    'For assigning SNVs (as determined by the Chip Array) as critical, the SNV genotype needs to differs from the ',
    'reference sample (if one is defined) and (by default preselection) it needs to change the protein sequence. ',
...
) %>% collapsible_html_text('Summary table explanations')
...
cat(str_glue(
    'This section includes all regions of interest defined for {sample_name}. ',
...
paste(
    'This table lists all SNVs/SNPs detected by the Chip Array which are different from the reference genome and',
    'are annotated as at least protein changing. The "SNV label" further categorizes the SNVs into critical,',
    'unreliable critical (due to missing reference or bad quality calls), protein changing, and reference genotype.\n\n',
    'The table allows sorting and filtering the SNPs by various criteria, default is sorting by the `SNV Label`.',
    'Hovering over the column headers gives explanations for each column and the "Column visibility" button can be',
    'used to show (or hide) columns.',    
    'I.e. the "Critical reason" column, which contains a text label showing why an SNV is labelled critical.',
    'This information is displayed by the color highlights of these columns:\n\n',
    ' - "ROI Hits", displaying overlap with sample specific annotation\n\n',
    ' - "Gene Name", hits in genes thta have known hotspots for stem cells, see also the next "SNV hotspot coverage" tab\n\n',
    ' - "Annotation", specific effect of the SNP on the gene based on mehari\n\n',
    ' - "HGVS.p", specific mutation in the proteion, which may overlap with a known stemcell hotspot (see also next tab)\n\n'
...
SNV_table_output(
    ...
    config, report_config, params$out_format #, caption
)
...
paste(
    'The following plots each show a whole chromosome overview of the sample, combing to a whole genome view.',
...
call_filter_regex <- ifelse(
    is.null(report_config$genome_overview$call.exclude_filter),
    'dummy',
    report_config$genome_overview$call.exclude_filter %>% paste(collapse = '|')
)
filtered_calls <- cnv_calls %>%
    filter(!str_detect(FILTER, call_filter_regex))
