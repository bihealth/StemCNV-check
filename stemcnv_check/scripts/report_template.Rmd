...
#TODO: replace these config functions if the yaml anchoring works
...
#     }
#     # empty lists are miss-represented in the config object created by snakemake
#     if (is.null(subconfig) & args[length(args)] %in% c('exclude_sections', 'sample.info.extra.cols', 
#                                                        'exclude_filter', 'always_include')) {
#         # nested lists, the other two are already the toplevel of report_settings
#         if (args[length(args)] %in% c('exclude_filter', 'always_include')) {
#             for (arg in args[1:(length(args)-1)]) {
#                 toplevel <- toplevel[[arg]]
#             }
#         }
#         # these to lists are not nested inside the report_settings toplevel
#         if (args[length(args)] %in% names(toplevel) ) {
#             subconfig <- list()
#         }
#     }    
#     subconfig
...
#   args <- list(...)
#   #Check if retry for '_default_' plot-section is needed
#   if (args[[1]] == 'call.data.and.plots') {
#     retry_default <- args[[2]] != 'filter-settings'
#     retry_args <- args
#     retry_args[[2]] <- '_default_'
#   } else {
#     retry_default <- FALSE
#   }
...
#   if (is.null(out.value)) out.value <- config_parse(args, '_default_')
#   if (is.null(out.value) & retry_default) out.value <- config_parse(retry_args, '_default_')
#   # Setting not found - error
#   if (is.null(out.value)) {
#     stop(paste0('Tried to find non-existing config settings: reports$',
...
report_settings <- params$report_config %>%
...
CNV_ID_str <- function(Call_label, CNV_type, chrom, Size, i,
                       region_name = NULL,
                       formatting = 'nice', ...) {
  ...
  # All other calls
  if (formatting == 'nice') {
    return(str_glue('Nr-{i} {CNV_type}: {chrom} - {format_size(Size)}'))
  } else {
    return(str_glue('nr-{i}-{CNV_type}-{chrom}---{format_size(Size)}') %>%
             str_to_lower())
  }
...
make_CNV_plot_section <- function(call.table, plotsection = 'denovo') {
  ...
  for (i in call.table$i) {
    ...
    if(i <= report.setting('call.data.and.plots', plotsection, 'min_number_plots') | row$Call_label %in% always_include) {
      ...
      #TODO: replace this with HI/HL/ROI table
      # if (report.setting('call.data.and.plots', plotsection, 'include.call.table')) {
      #   subchunkify(simple_table_output(res$calls, params$out_format), str_glue('CNV_call.{plotsection}.nr{i}.table'))
      #   cat('\n\n')
      # }
...
denovo_calls.table <- cnv_calls %>% 
    ...
    filter(!str_detect(FILTER, report.setting('call.data.and.plots', 'denovo', 'exclude_filter') %>% paste(collapse = '|'))) %>%
    mutate(i = row_number())

reference_calls.table <- cnv_calls %>% 
    dplyr::rename(chrom = seqnames, Size = width) %>%
    arrange(desc(Check_Score), desc(Size), chrom, start) %>%
    mutate(Call_label = ifelse(is.na(Call_label), '', Call_label)) %>%
    filter(sample_id == !!sample_id & Call_label == 'Reference genotype') %>%
    filter(!str_detect(FILTER, report.setting('call.data.and.plots', 'reference_gt', 'exclude_filter') %>% paste(collapse = '|'))) %>%
    mutate(i = row_number())
