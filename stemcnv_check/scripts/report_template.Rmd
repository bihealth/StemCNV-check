...
raw_LRR_BAF <- file.path(datapath, use_ids, paste0(use_ids, '.processed-SNP-data.', plot.probe.filter.setting, '-filter.vcf')) %>%
    lapply(parse_snp_vcf, apply_filter = FALSE) %>%
    #TODO this might not work accordingly to tb
    bind_ranges() %>%
...
SNP.genotype.distances <- file.path(datapath, snp.ids, 
                                    paste0(snp.ids, '.processed-SNP-data.', cluster.probe.filter.setting, '-filter.vcf')) %>%
    lapply(
        parse_snp_vcf,
        format_fields = c('GT'),
