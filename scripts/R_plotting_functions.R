...
make_LRR_BAF_plots <- function(call.row,
  ...
  calls <- cnv_calls %>%
    filter(caller_merging_state != 'combined' & Chr == chr & End >= win_start & Start < win_end) %>%
  ...
  high_impact_list <- call.row$high_impact_hits %>% str_split(',') %>% unlist()
  highlight_list <- call.row$highlight_hits %>% str_split(',') %>% unlist()
  gene.data <- gr_genes %>%
  ...
  cnv_track <- ggplot(calls) +
    geom_tile(aes(x = x_pos, y = y_pos, width = Size, height = .9, fill = color)) +
    scale_fill_identity() +
  ...
  calls <- calls %>%
    filter(!is.na(x_pos)) %>%
    dplyr::select(Sample_Name, 1, 3:4, Call_Label, Impact_Score, reference_overlap, CNV_type, CNV_caller) %>%
    arrange(Sample_Name, Call_Label, Impact_Score, Start) %>%
    rowwise() %>%
