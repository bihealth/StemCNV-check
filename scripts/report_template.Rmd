...
gene_table_output <- function(tb, caption = NULL, extra_cols = c()) {
  ...
  tb <- tb %>%
    ...
    arrange(desc(high_impact), desc(direct_hit), start) %>%
    select(gene_name, gene_id, seqnames, start, end, high_impact, any_of(extra_cols), REEV, GTex, NCBI, Ensembl)
  if (params$out_format == 'html') {
    colors <- ifelse(tb$high_impact == '-', 'white', 'red')
    dt <- datatable(tb, rownames = FALSE, escape = FALSE,
        ...
        formatStyle('high_impact', backgroundColor = styleRow(1:nrow(tb), colors)) # textAlign = 'center'
    return(dt)
  } else {
    tb <- tb %>% select(gene_name, gene_id, high_impact, any_of(extra_cols))
    return(kable(tb, caption = caption))
  }
}
...
CNV_table_output <- function(tb, caption = NULL) {
  # Reorder & subset columns
    tb <- tb %>%
      select(sample_id, ID, Chr, Start, End, Size, reportable, CNV.state, tool,
             #curated.genes,
             high.impact, call.in.reference, high.impact.genes,
             copynumber, numsnp,  merged_tool_calls, tool_confidence,
             tool.coverage.overlap, tool.overlap.state,
             coverage.by.ref, ref.tool )
  ...
  if (params$out_format == 'html') {
    dt <- datatable(tb,
            ...
            options = list(
              ...
              columnDefs = list(
                ...
                list(targets = c(0,1,12:(ncol(tb)-1)), visible = FALSE)
              )
            )
          ) %>%
    ...
    tb <- tb %>% select(Chr, Start, End, Size, reportable, CNV.state, tool,
                        #curated.genes,
                        high.impact, call.in.reference, high.impact.genes
    )
    return(kable(tb, caption = caption))
...
cnv_calls <- lapply(file.path(datapath, use_ids, paste0(use_ids, '.combined-cnv-calls.tsv')),
  ...
  mutate(
      reportable = case_when(
        ...
        CNV.state %!in% c('gain', 'loss') & length >= critical.loh                       ~ 'critical',
        CNV.state  %in% c('gain', 'loss') & length >= critical.cnv                       ~ 'critical',
...
make_LRR_BAF_plots <- function(call.row,
  ...
  high_impact_list <- call.row$high.impact.genes %>% str_split(',') %>% unlist()
  gene.data <- gr_genes %>%
  ...
  gene.data <- gene.data %>%
    filter(!is.na(x_pos)) %>%
    dplyr::select(seqnames, start, end, width, strand, high_impact, direct_hit, gene_name, gene_type, gene_id) %>%
    unique()
