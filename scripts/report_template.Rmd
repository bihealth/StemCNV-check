...
format_column_names <- function(n) {
  str_replace(n, '_', ' ') %>% str_to_title() %>% str_replace('Cnv', 'CNV')
}
...
CNV_table_output <- function(tb, plotsection, caption = NULL) {
  # Reorder & subset columns
    tb <- tb %>%
      ...
      mutate(Plot = ifelse(
               i <= report.setting('call.data.and.plots', plotsection, 'min_number_plots') | Call_Designation == 'Critical Call',
               ...
               #TODO: does this work for ROIs?
               str_glue('<a href="" onclick="window.open(\'./{image_folder}/CNV_call.{plotsection}.nr{i}.plot-1.png\'); return false;">Plot (ext.) {i}</a>')
             )
      ) %>%
      select(sample_id, ID, i, #invis 0-2
             Plot, Call_Designation, Impact_Score,
             CNV_type, Chr, Start, End, Size, #invis 6-10
             CNV_caller, high_impact_hits, highlight_hits,
             Precision_Estimate, probe_coverage_gap, high_probe_density,
             # invis: 17++
             copynumber, n_snp_probes,  n_premerged_calls, caller_confidence,
             caller_merging_coverage, caller_merging_state,
             reference_overlap, reference_coverage, reference_caller, percent_gap_coverage )
    ...
    dt <- datatable(tb %>% rename_with(format_column_names),
            ...
            options = list(
              ...
              columnDefs = list(
                ...
                list(targets = c(0:2,6:10,17:(ncol(tb)-1)), visible = FALSE)
              )
            )
          ) %>%
...
make_CNV_plot_section <- function(call.table, plotsection = 'denovo') {
  ...
  for (i in call.table$i) {
    ...
    if(i <= report.setting('call.data.and.plots', plotsection, 'min_number_plots') | row$Call_Designation == 'Critical Call') {
...
cnv_calls <- lapply(file.path(datapath, use_ids, paste0(use_ids, '.combined-cnv-calls.tsv')),
  ...
  mutate(
    Call_Designation = case_when(
            ...
            Impact_Score >= report.setting('min.impact.critical')   ~ 'Critical Call',
            Impact_Score >= report.setting('min.impact.reportable') ~ 'Reportable Call',
            TRUE                         				            ~ 'de-novo Call'
    )
  )
...
metric.colors <- list(
    ...
    mutate(metrics[[4]],
                 ...
                 # This is already part of the critical / reportable Impact_Score
...
make_LRR_BAF_plots <- function(call.row,
  ...
  calls <- calls %>%
    filter(!is.na(x_pos)) %>%
    dplyr::select(Sample_Name, 1, 3:4, Call_Designation, Impact_Score, reference_overlap, CNV_type, CNV_caller) %>%
    arrange(Sample_Name, Call_Designation, Impact_Score, Start) %>%
    rowwise() %>%
...
cnv_calls <- cnv_calls %>%
  ...
  arrange(desc(Impact_Score), desc(Size), Chr, Start)

denovo_calls.table <- cnv_calls %>%
  filter(caller_merging_state != 'pre-overlap'  &
           sample_id == !!sample_id &
           Call_Designation != 'Reference genotype'
  ) %>%
  mutate(i = row_number())
