...
CNV_table_output <- function(tb, caption = NULL) {
  # Reorder & subset columns
    tb <- tb %>%
      # works, but doesn't actuallly hightlight the correct tab
...
metrics <- list(
    ...
    cnv_calls %>%
        ...
        group_by(sample_id, loss_gain_log2ratio, highlight_calls, high_impact_calls, class, .drop = F) %>%
...
```{r primary.cnvs.header, eval = include.section('primary.cnvs.table') | include.section('primary.cnvs.plots'), results='asis'}
...
```{r primary.cnvs.table, results='asis', eval = include.section('primary.cnvs.table')}
...
```{r primary.cnvs.plots, eval = include.section('primary.cnvs.plots'), results='asis'}
make_CNV_plot_section(primary.call.table, 'primary')
cat('\n\n')
```

```{r secondary.cnvs.header, eval = include.section('secondary.cnvs.table') | include.section('secondary.cnvs.plots'), results='asis'}
...
```{r secondary.cnvs.table, eval = include.section('secondary.cnvs.table'), results='asis', cache=FALSE}
CNV_table_output(secondary.call.table, caption = 'Secondary calls')
cat('\n\n')
```

```{r secondary.cnvs.plots, eval = include.section('secondary.cnvs.plots'), results='asis'}
make_CNV_plot_section(secondary.call.table, 'secondary')
cat('\n\n')
```

```{r reference_gt.cnvs.header, eval = include.section('reference_gt.cnvs.table') | include.section('reference_gt.cnvs.plots'), results='asis'}
...
```{r reference_gt.cnvs.table, eval = include.section('reference_gt.cnvs.table'), results='asis', cache=FALSE}
CNV_table_output(reference_gt.call.table, caption = 'Reference genotype calls')
cat('\n\n')
```

```{r reference_gt.cnvs.plots, eval = include.section('reference_gt.cnvs.plots'), results='asis'}
