...
# highlight_warnings <- report.setting('SampleQCwarnings', 'highlight_calls')
# high_impact_warnings <- report.setting('SampleQCwarnings', 'high_impact_calls')
...
CNV_ID_str <- function(Call_Designation, CNV_type, Chr, Size, i,
                       region_name = NULL,
                       formatting = 'nice', ...) {
  ...
  # All other calls
  type_str <- case_when(
    Call_Designation == 'Origin Genotype' ~ 'RefGT',
    Call_Designation == 'Critical Call' ~ 'Crit',
    Call_Designation == 'Reportable Call' ~ 'Rep',
    Call_Designation == 'Secondary Call' ~ 'Sec'
  )
  if (formatting == 'nice') {
    return(str_glue('{type_str}-{i} {CNV_type}: {Chr}-{format_size(Size)}'))
  } else {
    return(str_glue('{type_str}-{i}-{CNV_type}-{Chr}-{format_size(Size)}') %>%
             str_to_lower())
  }
...
CNV_table_output <- function(tb, caption = NULL) {
  # Reorder & subset columns
    tb <- tb %>%
      # The links work, but switching the active tab requires more JS, sadly the Rmd tabs do not have IDs making this very tricky
      mutate(Call_Details = paste0('<a data-toggle="tab" href="#', pmap_chr(., CNV_ID_str, formatting = 'link'), '"',
                                   # # This part doesn't seem to do anything (or the same thing as <data-toggle="tab">)
                                   # ' onclick="$(', "'#", pmap_chr(., CNV_ID_str, formatting = 'link'), "').trigger('click')", '"',
                                   ">",
                                   pmap_chr(., CNV_ID_str), '</a>')) %>%
      select(sample_id, ID, i, Call_Designation, Call_Details,
             ImpactScore, ReliabilityScore, Chr, Start, End, Size, CNV_type, CNV_caller,
             high_impact, highlight, reference_overlap,
             call_has_probe_gap, high_probe_density,
             ...
             reference_coverage, reference_caller, percent_gap_coverage )
  ...
  if (params$out_format == 'html') {
    dt <- datatable(tb,
            ...
            options = list(
              ...
              columnDefs = list(
                ...
                list(targets = c(0,1,2,3,8,9,20:(ncol(tb)-1)), visible = FALSE)
              )
            )
          ) %>%
    ...
    tb <- tb %>% select(Call_Designation, ImpactScore, ReliabilityScore,
                        Chr, Start, End, Size, CNV_type, CNV_caller,
                        high_impact, highlight, reference_overlap,
                        call_has_probe_gap, high_probe_density,
                        high_impact_genes, highlight_genes
    )
...
make_CNV_plot_section <- function(call.table, plotsection = 'primary') {
  ...
  for (i in call.table$i) {
      ...
      if (report.setting('call.data.and.plots', plotsection, 'include.plot')) {
        subchunkify(res$gg, paste0(plotsection, '.call.',i,'.plot'), 5, 10)
        cat('\n\n')
      }
      if (report.setting('call.data.and.plots', plotsection, 'include.call.table')) {
        subchunkify(simple_table_output(res$calls), paste0(plotsection, '.call.',i,'.table'))
        cat('\n\n')
      }
      if (report.setting('call.data.and.plots', plotsection, 'include.gene.table.details') != 'None') {
        gene.tb <-  res$genes
        subchunkify(gene_table_output(gene.tb, plotsection), paste0(plotsection, '.call.',i, '.table.genes'))
        cat('\n\n')
      }
      cat('\n\n')
  }
...
metrics <- list(
    ...
    cnv_calls %>%
        ...
        mutate(class = factor(ifelse(CNV_type %!in% c('gain', 'loss'), 'LOH', 'CNV'), levels = c('CNV', 'LOH')),
               ...
               # high_impact_calls = sum(!is.na(high_impact))
        ) %>%
        group_by(sample_id, loss_gain_log2ratio,
                 #highlight_calls, high_impact_calls,
                 class, .drop = F) %>%
        summarise(total_calls = n(),
                  reportable_calls = sum(Call_Designation == 'Reportable Call'),
                  critical_calls = sum(Call_Designation == 'Critical Call'),
                  ) %>%
...
apply_gr_th <- function(data, warning_levels, second = 'orange'){
...
metric.colors <- list(
    ...
    mutate(metrics[[3]],
                 SNP_distance_to_reference = ifelse(SNP_distance_to_reference == '-', 'white',
                                              apply_gr_th(SNP_distance_to_reference, SNP_distance_to_ref_warning,
                                                          warn_color('SNP_distance_to_ref_warning'))
                 )),
    mutate(metrics[[4]],
                 loss_gain_log2ratio = apply_gr_th(abs(loss_gain_log2ratio), ratio_CNV_warnings, 'loss_gain_log2ratio'),
                 total_calls_CNV = apply_gr_th(total_calls_CNV, total_CNV_warnings, 'total_calls_CNV'),
                 total_calls_LOH = apply_gr_th(total_calls_LOH, total_LOH_warnings, 'total_calls_LOH'),
                 reportable_calls_CNV = apply_gr_th(reportable_calls_CNV, reportable_CNV_warnings, 'reportable_calls_CNV'),
                 reportable_calls_LOH = apply_gr_th(reportable_calls_LOH, reportable_LOH_warnings, 'reportable_calls_LOH'),
                 critical_calls_CNV = apply_gr_th(critical_calls_CNV, critical_CNV_warnings, 'critical_calls_CNV'),
                 critical_calls_LOH = apply_gr_th(critical_calls_LOH, critical_LOH_warnings, 'critical_calls_LOH'),
                 # This is already part of the critical / reportable ImpactScore
                 # highlight_calls = apply_gr_th(highlight_calls, highlight_warnings, 'highlight_calls'),
                 # high_impact_calls = apply_gr_th(high_impact_calls, high_impact_warnings, 'high_impact_calls')
                )
        )
...
get_call_stats_tool <- function(use.tool) {
    cnv_calls %>%
        ...
        summarise(total_calls = n(),
                  reportable_calls = sum(Call_Designation == 'reportable'),
                  critical_calls = sum(Call_Designation == 'critical')
        ) %>%
...
make_LRR_BAF_plots <- function(call.row,
  ...
  calls <- cnv_calls %>%
    filter(tool.overlap.state != 'combined' & Chr == chr & End >= win_start & Start < win_end) %>%
  ...
  calls <- calls %>%
    filter(!is.na(x_pos)) %>%
    dplyr::select(Sample_Name, 1, 3:4, Call_Designation, ImpactScore, ReliabilityScore, reference_overlap, CNV_type, CNV_caller) %>%
    arrange(Sample_Name, Call_Designation, ImpactScore, Start) %>%
    rowwise() %>%
...
cnv_calls <- cnv_calls %>%
  ...
  mutate(CNV_type = factor(CNV_type),
         reference_overlap = factor(reference_overlap),
           # These are list columns
           #high_impact = factor(high_impact),
           #highlight = factor(highlight),
  ) %>%
  arrange(desc(ImpactScore), desc(Size), Chr, Start)

primary.call.table <- cnv_calls %>%
  filter(tool.overlap.state != 'pre-overlap' &
           sample_id == !!sample_id &
           Call_Designation %in% c('Critical Call', 'Reportable Call')
  ) %>%
  mutate(i = row_number())

secondary.call.table <- cnv_calls %>%
  filter(tool.overlap.state != 'pre-overlap' &
           sample_id == !!sample_id &
           Call_Designation == 'Secondary Call'
  ) %>%
  mutate(i = row_number())

reference_gt.call.table <- cnv_calls %>%
  filter(tool.overlap.state != 'pre-overlap' &
           sample_id == !!sample_id &
           Call_Designation == 'Origin Genotype'
  ) %>%
  mutate(i = row_number())

```

```{r primary_cnvs.header, eval = include.section('primary_cnvs.table') | include.section('primary_cnvs.plots'), results='asis'}
cat('## Critical and reportable CNV calls {.tabset}\n\n')
cat('This section includes all calls with critical or primary designation (=primary calls)\n\n')
```


```{r primary_cnvs.table, results='asis', eval = include.section('primary_cnvs.table')}
#cat('List of all primary CNV calls from the input sample.\n The pre-processing step merges overlapping calls from multiple callers with the same CNV_type after removing very small calls.\n\n')
CNV_table_output(primary.call.table, caption = 'Primary CNV calls')
cat('\n\n')
```



```{r primary_cnvs.plots, eval = include.section('primary_cnvs.plots'), results='asis'}
make_CNV_plot_section(primary.call.table, 'primary')
cat('\n\n')
```

```{r secondary_cnvs.header, eval = include.section('secondary_cnvs.table') | include.section('secondary_cnvs.plots'), results='asis'}
cat('## Secondary calls {.tabset}\n\n')
cat('This section includes call that are neither reportable, critical or matching the reference genotype\n\n')
```



```{r secondary_cnvs.table, eval = include.section('secondary_cnvs.table'), results='asis', cache=FALSE}
CNV_table_output(secondary.call.table, caption = 'Secondary calls')
cat('\n\n')
```

```{r secondary_cnvs.plots, eval = include.section('secondary_cnvs.plots'), results='asis'}
make_CNV_plot_section(secondary.call.table, 'secondary')
cat('\n\n')
```

```{r reference_gt.cnvs.header, eval = !is.na(ref_id) & (include.section('reference_gt.cnvs.table') | include.section('reference_gt.cnvs.plots')), results='asis'}
cat('## Reference Genotype calls {.tabset}\n\n')
cat('This section includes all calls that match the genotype of the reference sample\n\n')
```



```{r reference_gt.cnvs.table, eval = !is.na(ref_id) & include.section('reference_gt.cnvs.table'), results='asis', cache=FALSE}
CNV_table_output(reference_gt.call.table, caption = 'Reference genotype calls')
cat('\n\n')
```

```{r reference_gt.cnvs.plots, eval = !is.na(ref_id) & include.section('reference_gt.cnvs.plots'), results='asis'}
make_CNV_plot_section(reference_gt.call.table, 'reference_gt')
cat('\n\n')
```
...
if (any(!is.na(regions)) & length(regions)>0) {
  ...
  print(CNV_table_output(fake_call_tb, caption = 'Regions of Interest'))
