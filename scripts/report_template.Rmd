...
CNV_table_output <- function(tb, caption = NULL) {
  # Reorder & subset columns
    tb <- tb %>%
      ...
      select(sample_id, ID, i, Call_Designation, Call_Details,
             Score, Chr, Start, End, Size, CNV_type, CNV_caller,
  ...
  if (params$out_format == 'html') {
    dt <- datatable(tb,
            ...
            options = list(
              ...
              columnDefs = list(
                ...
                list(targets = c(0,1,2,3,7,8,19:(ncol(tb)-1)), visible = FALSE)
              )
            )
          ) %>%
    ...
    tb <- tb %>% select(Call_Designation, Score,
...
metric.colors <- list(
    ...
    mutate(metrics[[4]],
                 ...
                 # This is already part of the critical / reportable Score
...
make_LRR_BAF_plots <- function(call.row,
  ...
  calls <- calls %>%
    filter(!is.na(x_pos)) %>%
    dplyr::select(Sample_Name, 1, 3:4, Call_Designation, Score, reference_overlap, CNV_type, CNV_caller) %>%
    arrange(Sample_Name, Call_Designation, Score, Start) %>%
    rowwise() %>%
...
cnv_calls <- cnv_calls %>%
  ...
  arrange(desc(Score), desc(Size), Chr, Start)
