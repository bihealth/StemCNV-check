...
CNV_table_output <- function(tb, plotsection, caption = NULL) {
  ...
  tb <- tb %>%
    ...
    mutate(Plot = ifelse(
             ...
             paste0(str_glue('<a href="" onclick="window.open(\'./{image_folder}/CNV_call.{plotsection}.nr{i}.plot-1.png\'); return false;">', pmap_chr(., CNV_ID_str),' (ext. plot)</a>')
           )
    ) %>%
...
cnv_calls <- lapply(file.path(datapath, use_ids, paste0(use_ids, '.combined-cnv-calls.tsv')),
  ...
  mutate(
    Call_Label = case_when(
            ...
            Impact_Score >= report.setting('impact.score.critical')   ~ 'Critical',
            Impact_Score >= report.setting('impact.score.reportable') ~ 'Reportable',
            TRUE                         				              ~ ''
    )
  )
...
not_enabled_label <- c('dummy',
                       ifelse(!is.na(report.setting('impact.score.critical')), 'dummy', 'Critical'),
                       ifelse(!is.na(report.setting('impact.score.reportable')), 'dummy', 'Reportable')
