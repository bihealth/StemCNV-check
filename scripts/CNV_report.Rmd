...
exclude_regexes <- config$settings$report$CNV_plots$exclude_gene_type_regex %>%
        paste(collapse = '|')
gene_type_whitelist <- config$settings$report$CNV_plots$include_only_these_gene_types
...
if (!is.na(gene_type_whitelist) & length(gene_type_whitelist) > 0){
...
read_raw <- function(filename) {
    read_tsv(filename, show_col_types = FALSE) %>%
        ...
        mutate(sample_id = basename(filename) %>% str_remove('.processed-data.tsv$'),
...
make_LRR_BAF_plots <- function(chr, start, end, size = 1e6, min_size=1e6) {
  ...
  if (min_size < 1e4) {
    ...
    if (nrow(plot.data)==0){
      warning(str_glue('No SNP probes found in primary plot area: {chr}:{start}-{end} +- {round(size/1e3)}kb'))
      return(list('gg' = 'No plot to show, without SNP probes in area', 'calls' = tibble(), 'genes' = tibble()))
    }
...
gg1 <- dend %>%
  ...
  ggplot(offset_labels = -max(SNP.genotype.distances)/25,
    ...
    scale_y_continuous(name = 'SNP distance (manhattan method)',
                                         ...
                                         limits = c(-max(SNP.genotype.distances)/2.5, NA),
                                         expand = expansion()) +
...
gg <- gg1 + as_ggplot(get_legend(gg2)) + plot_layout(ncol = 1, heights = c(4.5, 1))
