...
#TODO: this should be covfered by config check in the wrapper
...
gene_table_output <- function(tb, caption = NULL) {
  tb <- tb %>%
    arrange(desc(direct_hit)) %>%
    mutate(across(any_of(c('direct_hit', 'gene_type', 'strand')), ~ factor(.)))
  if (params$out_format == 'html') {
    return(datatable(tb, options = list(dom = 'pftil', pageLength = 10), rownames = FALSE))
  } else {
    return(kable(tb, caption = caption))
  }
}
...
make_LRR_BAF_plots <- function(chr, start, end, size = 1e6, total_min_size = 2e6, flank_factor = 2) {
  ...
  lrr <- ggplot(plot.data) +
    geom_rect(data = tibble(), aes(xmin = start, xmax = end, ymin = -1.5, yamx = 1.5), fill = 'grey50', alpha = 0.3) +
  ...
  baf <- ggplot(plot.data) +
    ...
    geom_rect(data = tibble(), aes(xmin = start, xmax = end, ymin = 0, yamx = 1), fill = 'grey50', alpha = 0.3) +
    scale_fill_identity() +
