...
params:
  ...
  sample_id: 'BIHi005-A-13'  #'GSM2538923_9371579017'
  sampletable: 'sample_table_example.txt' #'sample_table.txt'
  configfile: 'config_test.yaml' #'CytoSNP-config.yaml'
...
if (!is.na(ref_id)) {
	...
	names(sample_headers) <- c(sample_name, ref_name)
} else {
	sample_headers <- c(sample_name )
	names(sample_headers) <- c(sample_name )
}
...
Combined.metrics <- reduce(metrics, full_join, by = 'sample_id') %>% tr_tibble() #lapply(metrics, tr_tibble) %>% bind_rows()
Combined.colors <-  reduce(metric.colors, full_join, by = 'sample_id') %>% tr_tibble() #lapply(metric.colors, tr_tibble) %>% bind_rows()
...
export.tb <- full_join(
		reduce(metrics, full_join, by = 'sample_id') %>%
			mutate(across(.cols = everything(), ~as.character(.)))  %>%
		pivot_longer(-sample_id, names_to = 'Measure', values_to = 'Value'),
		reduce(metric.colors, full_join, by = 'sample_id') %>%
...
penncnv.stats <- lapply(logfiles, function(fname) {
	...
	notice_lines <- str_subset(lines, 'NOTICE:')
	# Extract from last line
	tb <- notice_lines[[length(notice_lines)]] %>%
			str_remove('.*: ') %>%
			str_split(' ') %>%
			unlist() %>%
...
```{r plots.roi, eval='Regions_of_Interest' %in% colnames(sampletable), results='asis'}
...
if (length(regions)>0) {
	for (region in regions) {
		...
		Chr <- ifelse(use_chr_str & str_detect(Chr, regex('chr', ignore_case = T)),
					   tolower(Chr), str_remove(Chr, regex('chr', ignore_case = T)))
		...
		res <- make_LRR_BAF_plots(Chr, Start, End, Size, 1e6)
		subchunkify(res$gg, paste0('plot.nonreportable.call.',i), 5, 10)
