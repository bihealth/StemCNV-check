...
params:
  ...
  sample_id: '205857150048_R01C01'  #'GSM2538923_9371579017'
  sampletable: 'sample_table.txt' #'sample_table.txt'
  configfile: 'default_config.yaml' #'CytoSNP-config.yaml'
  workdir: '/home/vonkunic_c/Misc-Projects/CNV-pipeline/data/205857150048_R01C01'
---
...
sampletable <- read_tsv(params$sampletable, col_types = 'cccccc')
...
ref_id <- ifelse(is.na(ref_name), NA, sampletable[sampletable$Sample_Name == ref_name, ]$Sample_ID)
...
reportable.loh <- config$settings$report$thresholds$reportable.loh
reportable.cnv <- config$settings$report$thresholds$reportable.cnv
fail.loh <- config$settings$report$thresholds$fail.loh
fail.cnv <- config$settings$report$thresholds$fail.cnv

#Warning tresholds
callrate_warnings <- config$settings$report$QCwarnings$callrate
total_CNV_warnings <- config$settings$report$QCwarnings$total_CNV
total_LOH_warnings <- config$settings$report$QCwarnings$total_LOH
ratio_CNV_warnings <- config$settings$report$QCwarnings$loss_gain_ratio
sizable_CNV_warnings <- config$settings$report$QCwarnings$sizable_CNV
sizable_LOH_warnings <- config$settings$report$QCwarnings$sizable_LOH
SNP_distance_reference <- config$settings$report$QCwarnings$SNP_distance_reference
...
SNP.ignore.filters <- config$settings$report$SNP_comparison$ignore_filters
...
read_raw <- function(filename) {
	read_tsv(filename, show_col_types = FALSE) %>%
		...
		mutate(sample_id = basename(filename) %>% str_extract(paste0('^', valid_name)),
			   Chr = paste0('chr', Chr),
...
raw_LRR_BAF <- file.path(basepath, datapath, use_ids, paste0(use_ids, '.processed-data.tsv')) %>%
  lapply(read_raw) %>%
	bind_rows() %>%
	mutate(filter.passed = filter_functions[[use.filter]](`GenTrain Score`, Score))
...
read_snps <- function(fname) {
	read_tsv(fname, col_select = c(1, 2, 11), show_col_types = FALSE) %>%
		...
		mutate(GType = ifelse(GType == 'NC', NA, str_count(GType, 'A')),
			   sample_id = basename(fname) %>% str_extract(paste0('^', valid_name)))
}

snp.ids <- unique(c(use_ids, SNP.extra.ids))
snp_file_pattern <- ifelse(SNP.ignore.filters,
						   '.processed-data.tsv',
						   paste0('.filtered-data.', use.filter, '.tsv'))

SNP.genotype.distances <- file.path(basepath, datapath, snp.ids, paste0(snp.ids, snp_file_pattern)) %>%
  lapply(read_snps) %>%
	bind_rows() %>%
	pivot_wider(names_from = sample_id, values_from = GType, values_fill = NA) %>%
	filter(if_all(everything(), ~!is.na(.))) %>%
	dplyr::select(-Index, -Name) %>%
	rename_with(~ sampletable[match(., sampletable$Sample_ID),]$Sample_Name ) %>%
	t() %>%
...
#TODO externalise & import this
list_cols <- c('coverage.by.ref', 'tool', 'individual_calls', 'numsnp',
               'copynumbers', 'conf', 'overlap.coverage', 'ref.tool')
...
cnv_calls <- lapply(file.path(basepath, datapath, use_ids, paste0(use_ids, '.combined-cnv-calls.', use.filter, '.tsv')), load_annotate_cnv) %>%
	bind_rows()

## GenCall logs
GenCall.stats <-  file.path(basepath, datapath, use_ids, paste0(use_ids, '.stats.txt')) %>%
...
logfiles <- list.files(file.path(basepath, config$log_path, 'PennCNV', sample_id),
					   pattern = paste0(use.filter, '.error.log'), full.names = T)

penncnv.stats <- lapply(logfiles, function(fname) {
	chrs <- basename(fname) %>% str_extract('^[^\\.]+')
	lines <- readLines(fname)
	# Extract from last line
	tb <- lines[[length(lines)]] %>% str_remove('.*: ') %>%
			str_split(' ') %>%
			unlist() %>%
	...
	dplyr::rename(` ` = Name)
datatable(penncnv.stats, options = list(dom = 't', pageLength = ncol(penncnv.stats)), rownames = FALSE)

# Number of CNVs, LOHs called at which thresholds
tbout <- get_call_stats_tool('PennCNV')
datatable(tbout, options = list(dom = 't', pageLength = ncol(tbout)), rownames = FALSE)
...
make_LRR_BAF_plots <- function(chr, start, end, size = 1e6, min_size=1e6) {
	...
	header <- ggplot(calls) + facet_wrap(~SampleName, nrow=1) + theme_classic() + 
	...
	calls <- calls %>% 
		filter(!is.na(x_pos)) %>%
		dplyr::select(SampleName, 1, 3:4, reportable, call.in.reference, CNV.state, tool) %>%
		#dplyr::rename(Chr = seqnames) %>%
		arrange(SampleName, start) %>% #Chr
		rowwise() %>%
		mutate(across(c(CNV.state, tool), ~paste(., collapse = ',')))

	gene.data <- gene.data %>% 
		filter(!is.na(x_pos)) %>%
		dplyr::select(start, end, strand, gene_name, gene_type, gene_id) %>%
		unique()
...
gg1 <- dend %>%
  ...
  ggplot(offset_labels = -max(SNP.genotype.distances)/25,
	...
	scale_y_continuous(name = 'SNP distance (manhattan method)',
										 ...
										 limits = c(-max(SNP.genotype.distances)/3, NA),
										 expand = expansion()) + 
...
gg2 <- dend.format.df %>%
  #arrange(SampleGroup) %>%
  ggplot(aes(x = SampleName, y = 1, col = !!sym(color_by), shape = !!sym(shape_by))) + 
  geom_point() + 
...
subchunkify(gg, 'snp.dendrogram', 7.5, 10)
