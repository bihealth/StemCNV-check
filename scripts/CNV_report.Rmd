...
params:
  ...
  out_format: 'html'
  configfile: 'test/test_config.yaml'
...
include.section <- function(section) {
  ...
  #exclude QC too sections if they are not defined in settings
...
fail.loh <- report.setting('thresholds', 'fail.loh')
fail.cnv <- report.setting('thresholds', 'fail.cnv')
...
if (include.section('SNP.dendrogram')) {
    extra_samples <- report.setting('SNP_comparison', 'extra_samples')
...
gene_table_dt <- function(tb) {
  tb %>% mutate(across(any_of(c('direct_hit', 'gene_type', 'strand')), ~ factor(.))) %>%
    datatable(options = list(dom = 'ftilp', pageLength = 20), rownames = FALSE)
}
...
get_call_stats_tool <- function(use.tool) {
    cnv_calls %>%
        filter(tool.overlap.state != 'combined' & use.tool %in% tool) %>%
        group_by(sample_id) %>%
...
#For pdf: library(condformat)
# https://stackoverflow.com/questions/45311909/how-do-you-color-the-cell-in-rmarkdown-pdf-output

  cat('No colored tables in pdf yet, sry :( \n\n')
...
```{r qc.GADA, eval = include.section('QC.GADA'), results='asis'}
cat('### GADA\n\n')
...
make_LRR_BAF_plots <- function(chr, start, end, size = 1e6, total_min_size = 2e6, flank_factor = 2) {
  ...
  #MaybeTODO: use actual chromosome size here
  ...
  gene_track <- ggplot(gene.data) +
    geom_tile(aes(x = x_pos, y = y_pos, width = width, height = .9), fill = 'grey30') +
...
if (params$out_format == 'html') {
  ...
  subchunkify(simple_table_output(reportable.calls %>% dplyr::select(-(12:18)),
                                   caption = 'Critical & reportable calls:'),
               'critical.call.table')
...
if (nrow(reportable.calls)>0) {
  for (i in 1:nrow(reportable.calls)) {
    ...
    cat(paste0('### ', row$CNV.state, ' ', state.str, ': ', row$Chr, ' - ', format_size(row$Size), '\n\n'))
    ...
    if (report.setting('call.data.and.plots', 'include.gene.table.details') != 'None') {
      ...
      subchunkify(simple_table_output(gene.tb), paste0('table.reportable.genes.',i))
      cat('\n\n')
    }
    cat('\n\n')
  }
...
if (params$out_format == 'html') {
  ...
  cat('Non reportable calls:\n\n')
  non.reportable.calls %>% dplyr::select(-(12:18)) %>% kable()
}
cat('\n\n')
```
...
if (nrow(non.reportable.calls)>0) {
  for (i in 1:nrow(non.reportable.calls)) {
    ...
    state.str <- ifelse(row$reportable %in% c('critical', 'yes, in ref.', 'no, in ref.'),
                        paste0(' (', str_remove(row$reportable, '(yes|no), '), ')'), '')
    cat(paste0('### ', row$Chr, ' - ', format_size(row$Size), ': ', row$CNV.state, state.str, '\n\n'))
    ...
    if (report.setting('call.data.and.plots', 'include.gene.table.details') != 'None') {
      ...
      subchunkify(simple_table_output(gene.tb), paste0('table.nonreportable.genes.',i))
      cat('\n\n')
    }
    cat('\n\n')
  }
...
if (!is.na(regions) & length(regions)>0) {
  for (region in regions) {
    ...
    if (report.setting('call.data.and.plots', 'include.gene.table.details') != 'None') {
      ...
      subchunkify(simple_table_output(gene.tb), paste0('table.roi.', region))
      cat('\n\n')
    }
  }
} else {
...
# UCSC should have cytoband & centromer positions available
...
if (params$out_format == 'html') {
  ...
  #Somehow tex/whatever executes from somewhere else then knitr and cna't find this ...
  include_graphics(png_file1)
}
