...
probe.filter <- config$settings$report$`plot-filter-settings`
...
raw_LRR_BAF <- file.path(basepath, datapath, use_ids, paste0(use_ids, '.processed-data.tsv')) %>%
  lapply(read_raw) %>%
    bind_rows() %>%
    mutate(filter.passed = filter_functions[[probe.filter]](`GenTrain Score`, Score))
...
clustering.filter <- config$settings$report$SNP_comparison$`filter-settings`
snp_file_pattern <- ifelse(clustering.filter %!in% names(filter_functions),
                           '.processed-data.tsv',
                           paste0('.filtered-data-', clustering.filter, '.tsv'))
...
cnv_calls <- lapply(file.path(basepath, datapath, use_ids, paste0(use_ids, '.combined-cnv-calls.tsv')),
                    load_annotate_cnv) %>%
    bind_rows()
...
logfiles <- list.files(file.path(basepath, config$log_path, 'PennCNV', sample_id),
                       pattern = 'error.log', full.names = T)
