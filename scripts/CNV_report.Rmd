...
report.setting <- function(...) {
  ...
  if (!any(is.na(as.numeric(subconfig)))) {
...
include.section <- function(section) {
  include <- ifelse(report.setting('include_sections') == '__all__', T, section %in% report.setting('include_sections'))
  exclude <- section %in% report.setting('exclude_sections')
  #TODO: don't hardcode available tools?
  include <- include & !(str_detect(section, 'QC\\.(CBS|PennCNV|GADA)') & str_remove(section, 'QC\\.') %!in% cnv_tools)
  include & !exclude
}
...
```{r qc.GADA, eval = include.section('QC.PennCNV'), results='asis'}
cat('### GADA\n\n')
...
```{r reportable, eval = include.section('reportable.table')}
...
if (nrow(reportable.calls)>0) {
  for (i in 1:nrow(reportable.calls)) {
    ...
    #TODO: should this be DT calls instead?
    ...
    if (report.setting('call.data.and.plots', 'include.gene.table.details') != 'None') {
      ...
      subchunkify(gene.tb, paste0('table.reportable.genes.',i))
    }
    cat('\n\n')
  }
} else {
...
if (nrow(non.reportable.calls)>0) {
  for (i in 1:nrow(non.reportable.calls)) {
    ...
    if (report.setting('call.data.and.plots', 'include.gene.table.details') != 'None') {
      ...
      subchunkify(gene.tb, paste0('table.nonreportable.genes.',i))
    }
    cat('\n\n')
  }
} else {
...
```{r rideogram, eval = include.section('virtual.karyotype'), results='asis'}
