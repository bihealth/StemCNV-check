...
params:
  ...
  sample_id: '207172180149_R02C02'  #'GSM2538923_9371579017'
  ...
  workdir: '/home/vonkunic_c/Misc-Projects/CNV-pipeline/data/207172180149_R02C02'
---
...
config_val <- function(value, section, default=NULL, category = 'report') {
	val=config$settings[[category]][[section]][[value]]
	if (is.null(val)) {
		return(default)
	} else {
		return(val)
...
reportable.loh = config_val('reportable.loh', 'thresholds', 2.5e6)
reportable.cnv = config_val('reportable.cnv', 'thresholds', 4e5)
fail.loh = config_val('fail.loh', 'thresholds', 5e6)
fail.cnv = config_val('fail.cnv', 'thresholds', 2e6)
...
SNP_distance_reference = config_val('SNP_distance_reference', 'QCwarnings', c(1, 5))

cnv_tool_order = config$settings$postprocessing$tool.order
cnv_tool_order = ifelse(is.null(cnv_tool_order), list(c('PennCNV', 'CBS')), list(c(cnv_tool_order))) %>% unlist()
...
SNP.extra.ids = c()
do.SNP.clustering = config_val('include_dendrogram', 'SNP_comparison', FALSE)
SNP.ignore.filters = config_val('ignore_filters', 'SNP_comparison', TRUE)
if (do.SNP.clustering) {
	extra_samples = config_val('extra_samples', 'SNP_comparison', c('__Chip_Name'))
...
#TODO need to ensure path is absolute, since rmd might have changed wd ?
...
read_snps <- function(fname) {
  read_tsv(fname, col_select = c(1, 2, 11), show_col_types = FALSE) %>%
    ...
    mutate(GType = ifelse(GType == 'NC', NA, str_count(GType, 'A')),
    			 sample_id = str_extract(fname, valid_name)) 
}
...
SNP.genotype.distances <- file.path(basepath, 'data', snp.ids, paste0(snp.ids, snp_file_pattern)) %>%
  lapply(read_snps) %>%
	bind_rows() %>%
	pivot_wider(names_from = sample_id, values_from = GType, values_fill = NA) %>%
	filter(if_all(everything(), ~!is.na(.))) %>%
	dplyr::select(-Index, -Name) %>%
	rename_with(~ sampletable[match(., sampletable$Sample_ID),]$SampleName ) %>%
	t() %>%
	...
	readRDS(fname) %>%
		...
		mutate(
			...
			reportable = case_when(
				...
				CNV.state == 'LOH' & length >= reportable.loh & call.in.reference ~ 'yes, in ref.',
				CNV.state != 'LOH' & length >= reportable.cnv & call.in.reference ~ 'yes, in ref.',
				CNV.state == 'LOH' & length >= fail.loh                           ~ 'critical',
				CNV.state != 'LOH' & length >= fail.cnv                           ~ 'critical',
				CNV.state == 'LOH' & length >= reportable.loh                     ~ 'yes',
				CNV.state != 'LOH' & length >= reportable.cnv                     ~ 'yes',
...
metrics <- list(
	...
	cnv_calls %>%
		...
		group_by(sample_id) %>%
		mutate(class = factor(ifelse(CNV.state == 'LOH', 'LOH', 'CNV'), levels = c('CNV', 'LOH')),
...
TODO: use a patched yaml that includes default values

```yaml
`r xfun::file_string(file.path(basepath, params$configfile))`
```
...
get_cnv_y <- function(cnv.state, tool) {
	...
	out <- ifelse(cnv.state == 'LOH' & tool == 'PennCNV', 0, out)
	out
}
...
make_LRR_BAF_plots <- function(chr, start, end, size = 1e6, min_size=1e6) {
	...
	calls <- cnv_calls %>%
		...
		mutate(x_pos = (end + start) / 2, 
					 ...
					 color = ifelse(CNV.state == 'loss', '#f46d43', '#1a9850'),
		...
		bind_rows(tibble(SampleName = factor(sample_headers, levels = sample_headers),
	...
	gene.data <- gr_genes %>%
		...
		mutate(x_pos = (end + start) / 2,
					 ...
					 SampleName = paste(sample_headers, collapse = '---')) %>%
...
color_by = config_val('dendrogram.color.by', 'SNP_comparison', 'SampleGroup') %>% check_cols_exits()
shape_by = config_val('dendrogram.shape.by', 'SNP_comparison', 'Chip_Name') %>% check_cols_exits()
