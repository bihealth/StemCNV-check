...
snv_cat_desc <- list(
    'ROI-overlap' = ' - overlaps with a sample specific region of interest',
    ...
    'protein-ablation' = ' - SNV that (likely) fully disrupts the protein function (i.e by frameshift or truncation)',
    'protein-changing' = ' - causes changes the protein sequence (i.e. point mutations)',
    'other' = ' - effect on protein can not be determined'
)
...
#TODO set headers / table names
# ? caption
# ? first col name >> inside fucntion
# ? text
summary_tables[[1]]

paste0(
    'For assigning SNVs (as determined by the Chip Array) as critical or reportable, the SNV genotype needs to differs ',
    'from the reference sample (if one is defined) and it needs to match specific criteria:\n\n',
    'Critical SNVs:\n\n', 
    paste(sapply(config$settings$SNV_analysis$critical_SNV, \(x){snv_cat_desc[[x]]}), collapse = '\n\n'), 
    '\n\nReportable SNVs:\n\n',
    paste(sapply(config$settings$SNV_analysis$reportable_SNV, \(x){snv_cat_desc[[x]]}), collapse = '\n\n'),
    '\n\n',
