...
paste(
    ...
    'The Check-Score is described on our upcoming manuscript and combines contributions from CNV size and copynumber ',
    'as well as additions from annotation from overlapping ',
    '<a href="https://bihealth.github.io/StemCNV-check/CNV-hotspots/index_1.html">stem cell hotspots</a>, ',
...
fake_call_tb <- roi_gr %>%
    ...
    annotate_gaps(
        fix_rel_filepath(params$array_gaps_file, config),
    ...
    annotate_high_density(
        fix_rel_filepath(params$array_density_file, config),
...
paste0(
    ...
    '\n\nThe following categories are assinged as "Critical" or "Reportable" (de-novo):\n\n',
    paste(imap(
        c(
            setNames(config$settings$SNV_analysis$critical_SNV, rep('Critical de-novo', length(config$settings$SNV_analysis$critical_SNV))),
            setNames(config$settings$SNV_analysis$reportable_SNV, rep('Reportable de-novo', length(config$settings$SNV_analysis$reportable_SNV)))
        ),\(x, name) str_glue(' - {name}: {x}')
        ), collapse = '\n\n'
    ),
    '\n\nA complete, up-to-date list of all stem cell SNV hotspots is also available ',
    '<a href="https://bihealth.github.io/StemCNV-check/SNV-hotspots/index_1.html">online</a>.',
...
if (!is.na(ref_id)) {
    label_SNVs <- SNV_table %>%
        ...
        mutate(
            sample_id = sample_id,
            color = ifelse(SNV_label == 'Critical de-novo', 'red', 'orange')
        )
} else {
